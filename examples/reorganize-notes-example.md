# reorganize_notes Tool - Usage Examples

The `reorganize_notes` tool executes vault reorganization based on an OrganizationPlan. It applies move and rename operations to improve vault organization with comprehensive safety features.
...
const dryRunResult = await reorganizeNotes({
  project_folder: 'my-project',
  plan: {
    suggestions: [
      {
...
const executionResult = await reorganizeNotes({
  project_folder: 'my-project',
  plan: {
    suggestions: [
      {
...
The `reorganize_notes` tool is designed to work with plans generated by `analyze_vault_structure`:
